...
public class MapIO {
    ...
    static {
        ...
        // 'A' ... 'Z' = 0 ... 25
        for (int i = 'A'; i <= 'Z'; i++) {
            base64Codes[i] = (byte) (i - 'A');
        }
        // 'a' ... 'z' = 26 ... 51
        for (int i = 'a'; i <= 'z'; i++) {
            base64Codes[i] = (byte) (26 + i - 'a');
        }
        // '0' ... '9' = 52 ... 61
        for (int i = '0'; i <= '9'; i++) {
            base64Codes[i] = (byte) (52 + i - '0');
        }
        // '+' = 62
        base64Codes['+'] = 62;
        // '/' = 63
     ...
     *
     * Usage (Image): BufferedImage image = BUFFEREDIMAGE_CREATED_FROM_IMAGE;
     * ByteArrayOutputStream outputStream = new ByteArrayOutputStream();
     * ImageIO.write(image, FILE_EXTENSION, outputStream); char[] encodedImage =
     * MapIO.base64Encode(outputStream.toByteArray());
     *
     * Usage (String): String string = ""; char[] encodedString =
     * MapIO.base64Encode(string.getBytes());
     *
     * See documentation if you need to encode something else.
     *
     * Information about Base64 can be found at
     ...
     *
     * Usage (Image): char[] base64Chars = BASE64CHARS; byte[] imageBytes =
     * MapIO.base64Decode(base64Chars); InputStream in = new
     * ByteArrayInputStream(imageBytes); BufferedImage decodedImage =
     * ImageIO.read(in);
     *
     * Usage (String): char[] base64Chars = BASE64CHARS; byte[] stringBytes =
     * MapIO.base64Decode(base64Chars); String decodedString = new
     * String(stringBytes);
     *
     * Information about Base64 can be found at
     * http://en.wikipedia.org/wiki/Base64
    ...
    public static byte[] base64Decode(char[] data) {
        ...
        for (int i = 0; i < data.length; i++) {
            if (data[i] > 255 || base64Codes[data[i]] < 0) {
                inputLength--; // Ignore non-valid chars and padding
            }
        }
    ...
    public static void loadProjectAsXML(String fileName, MapEditor parentFrame) {
        ...
        try {
            ...
            if (temp.exists()) {
                Document doc = parser.build(temp);
